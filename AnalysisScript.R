...
## ##################################################################
## ### Glacier Bay sea otter analysis for Diversity and Distributions
## ##################################################################
## ###
...
## source(paste("~/Dropbox/Post-Doc/RFiles/EcologicalDiffusion/",
##              "HomoginizationFunctions.R",sep=""))
...
## ## ## 1993
## ## TP.surv.1993=subset(OrigData.tmp,OrigData.tmp$year==1999)
## ## l.1993=list()
## ## Sl.1993=list()
## ## S.1993=list()

## ## for(i in 1:dim(TP.surv.1993)[1]){
## ##     l.1993[[i]]=cbind(c(TP.surv.1993$west_long[i],
## ##                         TP.surv.1993$east_long[i]),
## ##                       c(TP.surv.1993$west_lat[i],
## ##                         TP.surv.1993$east_lat[i]))
## ##     Sl.1993[[i]]=Line(l.1993[[i]])
## ##     S.1993[[i]]=Lines(list(Sl.1993[[i]]), ID=i)
## ## }
## ## Sb.1993=SpatialLines(S.1993)
## ## ## plot(Sb.1993)
...
## gamma.tune=0.0006545584
## beta.tune=c(0.059049,0.01958004,0.04782969,0.02393116,0.0165908)
## theta.tune=20.9437
## kappa.tune=0.4094121
## odp.tune=0.01
...
## N=ifelse(N<Y,Y,N)
## ## N=rnbinom(n=length(lambda),size=odp,,mu=lambda)
## ## plot(N[N>0],lambda[N>0])
...
##                  n.tot.save
##                  )
##         save(out,file="~/Dropbox/Post-Doc/manuscripts/SeaOtterEcology/DandD/Revision/MCMCNBResults9.RData")
##         cat(k,"")
##     }
...
## ###
## ### Results Files:
## ###        MCMCRNBesults1.RData (06 Feb 2018): Sample only gamma (worked, maybe tiny bias high).
## ###        MCMCRNBesults2.RData (06 Feb 2018): Sample gamma and beta (worked).
## ###        MCMCRNBesults3.RData (06 Feb 2018): Same as 2, but sampled initial conditions.
## ###        MCMCRNBesults4.RData (06 Feb 2018): Same as 3, but sampled N. (not working).
## ###        MCMCRNBesults5.RData (06 Feb 2018): Same as 1, but sampled N. (clear bias low).
## ###        MCMCRNBesults6.RData (06 Feb 2018): Same as 1, but sampled gamma and N (worked).
## ###        MCMCRNBesults7.RData (06 Feb 2018): Same as 3, but sampled  N.
## ###        MCMCRNBesults8.RData (06 Feb 2018): Same as 7, but sampled  odp. (worked)
## ###        MCMCRNBesults9.RData (07 Feb 2018): Same as 8, but sampled  p varying by year
## ###                                            and derived n.tot (worked)
## ###
## #############################################################################################
...
## load("~/Dropbox/Post-Doc/manuscripts/SeaOtterEcology/DandD/Revision/MCMCNBResults9.RData")
...
## for(i in 1:20){
##     plot(density(N.est[,i]),xlim=c(0,10000))
## }
